...
import { RawSong, TrackId } from "./types";
...
export type CategoryToMembers = Record<IndexedCategory, CategoryMember[]>;

export type MemberToTracks = Record<CategoryMember, RawSong["_id"][]>;
...
export const categorySelectIndex = (
  ...
  const selectBy = r.curry(
    (category: IndexedCategory, terms: string[]): TrackId[] => {
      const idsFromTerm = (term: string) => selectIndex[category][term] || [];

      const ids = r.pipe<string[], TrackId[][], TrackId[]>(
        r.map(idsFromTerm),
        r.flatten
      )(terms);
