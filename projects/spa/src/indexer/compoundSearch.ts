...
import { CompoundSearchOpts, IndexInterface } from "./indexer";
import { IndexedCategory, SelectIndex, SelectOptions } from "./categoryIndex";
...
export const compoundSearcher = ({
  ...
  let yearFinds: TrackId[] | undefined;
  let searchFinds: TrackId[] | undefined;
  if (opts.searchQuery) {
...
const makeSelectOptionsForCategory = r.curry(
  (index: SelectIndex, category: IndexedCategory, finds: TrackId[]) =>
    r.mapObjIndexed(
...
const intersectedTracks = (ids: (TrackId[] | undefined)[]) => {
  const intersectableTracks = r.reject(r.isNil, ids) as TrackId[][];
  switch (intersectableTracks.length) {
    ...
    default:
      ...
      return r.reduce<TrackId[], TrackId[]>(
        r.intersection,
