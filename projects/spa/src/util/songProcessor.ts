...
type MemberToTracks = Record<CategoryMember, RawSong["trackId"][]>;
...
const makeSelectIndex = (allSongs: RawSong[]): SelectIndex => {
  ...
  for (let { trackId, singer, orchestra, genre } of allSongs) {
    if (singer !== "") {
      options.singer = {
        ...options.singer,
...
const indexSongs = (rawSongs: RawSong[]): IndexedSongData => {
