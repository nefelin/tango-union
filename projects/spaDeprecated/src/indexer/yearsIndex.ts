...
import type {TrackId, JustYear} from './types';
...
const tokenize = (query: string): Array<Token> => {
...
const expandTokens = (tokens: Array<Token>): Maybe<YearFilter> => {
...
export const yearsIndex = (rawSongs: Array<JustYear>) => {
  const songsByYears = (query: string): Array<TrackId> => {
